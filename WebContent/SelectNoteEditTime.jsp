...
<nav class="navbar navbar-default navbar-static-top">
 ...
 <%
	...
	rs=sql.executeQuery("select * from note where authorname= '"+usernamevalue+"'order by lastedittime desc");
	...
	<div class="row">
		<div style="float:left">
   		...
   		<div style="float:left">
   			<br>
   			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   			<a href="SelectNoteEditTime.jsp?username=<%=usernamevalue%>" class="btn btn-mysample1 active">sort by lastedittime desc</a>
   		</div>
   		<form action="#" method="post">
   		<!-- <div style="float:left;margin-top:3%">
   			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   			<select name="NoteLabel" style="width:50px;height:6px;">
		...
		<div class="col-md-3" style="height:150px;">
			<div class="row no-gutters border-primary rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
				<div class =col p-4 d-flex flex-column position-static>
					<h3 class="mb-0"><%=rs.getString(2)%></h3>
					<h7>label:<%=rs.getInt(7) %></h7>
					<div class="mb-1 text-muted">create:<%=rs.getDate("createtime")%></div>
					<div class="mb-1 text-muted">lastedit:<%=rs.getDate("lastedittime")%></div>
					<div class="test" style="text-overflow:ellipsis;">
					<%
					String htmlStr=rs.getString(3);
					String textStr ="";
					...
					<a href="ShowNoteDetail.jsp?noteid=<%=rs.getInt(1)%>&username=<%=usernamevalue%>&notetitle=<%=rs.getString(2)%>&notecontent=<%=rs.getString(3)%>&notelabel=<%=rs.getInt(7) %>" class="streched-link">continue reading &edit</a>
